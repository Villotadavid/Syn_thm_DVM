...
for idx in range(5):
    ...
    responses = client.simGetImages([airsim.ImageRequest("1", airsim.ImageType.DepthPerspective, True)])  #scene vision image in uncompressed RGBA array
    response=responses[0]
    img = airsim.list_to_2d_float_array(response.image_data_float, response.width, response.height)
    cv2.imshow('img',img)
    a, img = cv2.threshold(img, 15, 255, cv2.THRESH_BINARY)
    cv2.imshow('img',img)
    cv2.waitKey(10)
