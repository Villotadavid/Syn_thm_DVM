      ...
      name_(nh_private.getNamespace())
{
  ...
  //deep_network_sub_ = nh_.subscribe("bebop/image_raw", 1, &deepNavigation::imageCallback, this);
...
void deepNavigation::run(FILE *f )
{
  ...
  while (ros::ok())
  {
    ...
    desired_angular_velocity_ = (1.0 - alpha_yaw_) * desired_angular_velocity_ + alpha_yaw_ * steering_angle_; //Change this line if steering doesn't work
    ...
    fprintf(f,"%.3f, %.3f,  %.3f,   %.3f \n",probability_of_collision_, steering_angle_,desired_forward_velocity_,desired_angular_velocity_);
    
    img=ros::topic::waitForMessage<sensor_msgs::Image>(bebop/image_raw,ros::Duration(10))
    rate.sleep();
    ros::spinOnce();
...
int main(int argc, char** argv)
{
  ...
  FILE * f = fopen("/home/tev/Desktop/file.txt", "wt");
  if (f == NULL)
  {
  ...
  fprintf(f, "Coll.Probability    Steering   V.Forward   V.Angular \n");
