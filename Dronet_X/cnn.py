...
def trainModel(train_data_generator, val_data_generator, model, initial_epoch):
    ...
    model.alpha1 = tf.Variable(1, trainable=False, name='alpha1', dtype=tf.float32)
    #model.alpha2 = tf.Variable(1, trainable=False, name='alpha2', dtype=tf.float32)
    #model.beta = tf.Variable(0, trainable=False, name='beta', dtype=tf.float32)

    # Initialize number of samples for hard-mining
    model.k_mse1 = tf.Variable(FLAGS.batch_size, trainable=False, name='k_mse1', dtype=tf.int32)
    #model.k_mse2 = tf.Variable(FLAGS.batch_size, trainable=False, name='k_mse2', dtype=tf.int32)
    ...
    sgd =optimizers.Adam(decay=1e-5)
    
    # Configure training process
    model.compile(loss=[utils.hard_mining_mse(model.k_mse1)],
                        optimizer=sgd, loss_weights=[model.alpha1])
    print model.compile
...
def _main():
    ...
    print val_datagen.flow_from_directory
