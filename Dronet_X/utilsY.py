...
class DroneDirectoryIterator(Iterator):
    ...
    def _decode_experiment_dir(self, dir_subpath):
        ...
        X_filename = os.path.join(dir_subpath, "LabelsY.txt")
            ...
            for frame_number, fname in enumerate(sorted_files):
                ...
                if is_valid:
                    ...
                    self.filenames.append(os.path.relpath(absolute_path,
                            self.directory))
		    #print str(ground_truth[frame_number])+':->'+str(exp_type)+'->'+absolute_path
    ...
    def _get_batches_of_transformed_samples(self, index_array) :
        ...
        batch_x = np.zeros((current_batch_size,) + self.image_shape,
                dtype=K.floatx())
        batch_Xi = np.zeros((current_batch_size, 2,),
                dtype=K.floatx())
        batch_Yi = np.zeros((current_batch_size, 2,),
                dtype=K.floatx())
	    ...
	    #print '++++++++++++++++++++++++++++++++++++++++++'
            ...
            if self.exp_type[index_array[i]] == 1:
                ...
                batch_Xi[i,0] =1.0
                batch_Xi[i,1] = self.ground_truth[index_array[i]]
                batch_Yi[i] = np.array([1.0, 0.0])
		#print 'X' + str(batch_Xi[i])+'->'+str(batch_Yi[i])
            else:
                # Collision experiment (t=0)
                batch_Xi[i] = np.array([0.0, 0.0]) 
                batch_Yi[i,0] = 0.0
                batch_Yi[i,1] = self.ground_truth[index_array[i]]
		#print 'Y' + str(batch_Xi[i])+'->'+str(batch_Yi[i])
