...
class MyCallback(keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        self.model.alpha1.load(1.0, sess)
	#self.model.alpha2.load(0.0, sess)

        print(self.model.alpha1.eval(sess))
        #print(self.model.alpha2.eval(sess))
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if (epoch+1) % self.period == 0:
            filename = self.filepath + '/model_weights_X_' + str(epoch) + '.h5'
        ...
        self.model.k_mse1.load(int(np.round(mse_function)), sess)
	#self.model.k_mse2.load(int(np.round(mse_function)), sess)
