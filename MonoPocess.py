...
def find_center(stereo):
	...
	for i in range (0,xmax):
	   for n in range (0,ymax):
		if stereo[i,n]>30:                      #Transformacion de la imagen en binaria
			count+=1			#Cuenta el numero de pixels de colision
	...
	if count==0:					#En caso de que no haya pixels de colision evita un div por cero
		count=1
	#print (count)
	posx=posx/count+200				#Calculo de coordenadas del centroide
	posy=posy/count+200
	print (posx,posy)
	PCol=count/1600
	#print (PCol)
...
def main():
	...
	for k in range (0,number_exp):
		...
		if os.path.exists(general+"/LabelsX.txt"):
			f1=open("./Dataset/LabelsX.txt","w")
			f1.write('X Punto objetivo'+'\n')
			f2=open("./Dataset/LabelsY.txt","w")
			...
			for fr in range (0,number_files):
				...
				cv2.circle(stereo,(p_obj[1],p_obj[0]),10,(0,255,255),2)
				cv2.imshow('img',stereo)
				cv2.waitKey(2)
				n=n+1
				#frname='frame_{0:05d}'.format(n)+'.png'
				#cv2.imwrite('./Dataset/imagesP'+'/'+frname, img)
				f1.write(str(p_obj[0])+'\n') #str(p_obj[0])+'  ,'+str(p_obj[1])
				f2.write(str(p_obj[1])+'\n') #str(p_obj[0])+'  ,'+str(p_obj[1])
			f1.close()
			f2.close()
		else:
			f1=open("./Dataset/LabelsX.txt","w")
