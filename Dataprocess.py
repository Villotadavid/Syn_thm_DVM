...
def main():
	...
	for fr in range (0,number_files):
		...
		line1,line2,p,line_params=regression_lines(regression_points,stereo)
					#Hay que buscar el centroide 
		if regression_points.size!=320000:
			p_obj[0],p_obj[1]=calcular_distancia(vectz,line_params[2],line_params[3],p[0],p[1])
		else:
			p_obj[0],p_obj[1]=[200,200]
		...
		plt.imshow(stereo,'gray')
		#plt.show()
		plt.pause(.1)
		plt.draw()
		n=n+1
		f1.write(str(p_obj[0])+','+str(p_obj[1])+'\n') #str(p_obj[0])+'  ,'+str(p_obj[1])
	f1.close()
